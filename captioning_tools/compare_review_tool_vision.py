"""
Vision-inclusive-LoRA variant of compare_review_tool.py/compare_review_tool_r2.py:
compares existing sb_caption against sb_caption_ft_vision (vision-inclusive
model, finetune_checkpoints_vision/epoch_4, from
finetune/rerun_review_vision_sample.py's 25-video sample - the SAME sample
round 2 used) instead of round 1's sb_caption_ft or round 2's
sb_caption_ft_r2. Uses its own log files (swap_decisions_vision.txt,
swap_backup_log_vision.txt) so none of the three rounds' decisions collide.

Same UX as the other two: [k]eep/[s]wap/[q]uit/Backspace, flag keys with
[existing]/[new]/[both] tagging, 'x' for info.txt corrections. See
compare_review_tool.py's docstring for full details - only the target
folder/log paths differ here.

Usage (run from the repo root):
  cd /home/mobiltech/Desktop/re-label/vlm
  source activate qwen3vl
  python captioning_tools/compare_review_tool_vision.py
"""
import os
import subprocess
import sys
...
DECISIONS_LOG = os.path.join(REPO_ROOT, "finetune", "swap_decisions_vision.txt")
BACKUP_LOG = os.path.join(REPO_ROOT, "finetune", "swap_backup_log_vision.txt")
...
def show_pair(caption_type, original_text, new_text):
    ...
    print(original_text)
    print("-" * 80)
    print(f"{caption_type} - NEW (sb_caption_ft_vision, vision-inclusive LoRA):")
    print(new_text)
    print("-" * 80)
...
def ask_version_tag():
    print("  이 오류가 어디 있나요? [e] 기존(sb_caption)만  [n] 새 것(vision)만  [b] 둘 다")
    while True:
        k = getch()
        if k in ("e", "E"):
            return "existing"
...
def decide_one(video_path, caption_type, old_path, new_path, applied_flags):
    ...
    while True:
        ...
        if key == "k":
            ...
            print("  -> swapped in vision-inclusive version")
            return "next"
        elif key in ("\x7f", "\x08"):
            return "back"
        elif key == "q":
