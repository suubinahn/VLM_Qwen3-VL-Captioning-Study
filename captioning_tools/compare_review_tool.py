"""
Side-by-side comparison tool: existing sb_caption (base model) vs sb_caption_ft
(fine-tuned base+LoRA model, from finetune/rerun_review.py) - for deciding,
per video and per caption type, whether the fine-tuned output should replace
the existing one.

Never swaps automatically - every replacement is a human decision made while
watching the actual video. Before overwriting a caption file, the true
original text is appended to swap_backup_log.txt so a mistaken swap can be
recovered.

Decisions are logged to swap_decisions.txt (tab-separated: timestamp,
video_path, caption_type, decision) so the session can resume where it left
off - a (video, caption_type) is skipped once it has a decision, unless you
back up into it with Backspace, in which case pressing [k]/[s] again
overwrites the earlier decision (and the saved file) with the new choice.

Usage (run from the repo root, NOT from inside caption_tools/ - SAMPLE_ROOT
and other paths pulled in from caption_review_tool.py are relative to the
repo root):
  cd /home/mobiltech/Desktop/re-label/vlm
  source activate qwen3vl
  python caption_tools/compare_review_tool.py
"""
import os
import subprocess
import sys
...
DECISIONS_LOG = os.path.join(REPO_ROOT, "finetune", "swap_decisions.txt")
BACKUP_LOG = os.path.join(REPO_ROOT, "finetune", "swap_backup_log.txt")
FLAGGED_LOG = os.path.join(REPO_ROOT, "flagged_videos.txt")


def load_flag_history():
    """video_path -> list of (category, note) from flagged_videos.txt, so the
    reviewer can see at a glance whether this video was previously flagged
    (and for what) or passed cleanly - needed to correctly judge whether a
    difference between old/new caption is a fix, a regression, or neither."""
    history = {}
...
def get_original_text(video_path, caption_type, current_text):
    """The true pre-swap text for this video/caption_type. If it was ever
    swapped before, the real original lives in swap_backup_log.txt (the file
    on disk now holds the fine-tuned text instead) - without this, going back
    and re-deciding after an earlier swap would compare the fine-tuned text
    against itself instead of against the real original."""
...
def remove_matching_flag_line(video_path, category, note):
    """Removes one flagged_videos.txt line matching this exact
    video/category/note (ignoring its timestamp) - used so re-flagging the
    same category for the same item after going back with Backspace replaces
...
def apply_decision(video_path, caption_type, decision, original_text, new_text, old_path):
    """Writes the chosen version to disk and logs the decision. Safe to call
    repeatedly for the same (video, caption_type) after going back with
    Backspace - each call re-derives from the true original (via
    get_original_text) and the unchanged fine-tuned text, so re-deciding
    always overwrites correctly regardless of what was chosen before."""
    if decision == "keep":
...
def show_pair(caption_type, original_text, new_text):
    ...
    print(original_text)
    print("-" * 80)
    print(f"{caption_type} - NEW (sb_caption_ft, fine-tuned):")
    print(new_text)
    print("-" * 80)
...
def ask_version_tag():
    """Which version(s) a newly-noticed error applies to - existing
    (sb_caption), new (sb_caption_ft), or both. Without this, a flag logged
    during comparison can't later be distinguished from a flag logged before
    fine-tuning even existed, making it impossible to count how often an
    error category actually persists after fine-tuning."""
    print("  이 오류가 어디 있나요? [e] 기존(sb_caption)만  [n] 새 것(fine-tuned)만  [b] 둘 다")
    while True:
        k = getch()
        if k in ("e", "E"):
            return "existing"
...
def decide_one(video_path, caption_type, old_path, new_path, applied_flags):
    """Returns 'next', 'back', or 'quit'. applied_flags is a
    {(video_path, caption_type): {category: note}} dict shared across the
    whole run, tracking what THIS session has already flagged for this item -
    pressing the same flag key again after going back replaces that entry
    ...
    while True:
        ...
        if key == "k":
            ...
            print("  -> swapped in fine-tuned version")
            return "next"
        elif key in ("\x7f", "\x08"):
            return "back"
        elif key == "q":
            ...
            if not value:
                ...
                continue
            # log_info_correction() already overwrites by field per video (a
            # dict update, not an append), so repeat presses are inherently
            # safe with no extra tracking needed here.
...
def build_target_sequence():
    """Flat (video_path, caption_type) sequence, two entries per eligible
    video - flat rather than per-video so Backspace can step back one
    decision at a time instead of one whole video at a time."""
    sequence = []
